...
<body>
	...
	<div class="creFund-wrap">
		...
		<div class="crefund-content">
			...
			<div class="calculate-wrap">
				...
				<br>
				-->
				<form action="/saveCalculate.do" method="post" enctype="multipart/form-data">
					<!-- 
					<div class="represent-input">
						<label for="tfcRepCategory" class="form-label mt-4">사업자구분</label>
						<select class="form-select" id="tfcRepCategory" name="fcRepCategory">
						...
						<input type="text" name="fcRepName" class="form-control" id="tfcRepName" placeholder="대표자명 입력" >
						<br>
						<label class="form-label mt-4" for="tfcRepEmail">대표자 이메일</label>
						<p>펀딩하기 서비스 제공 시 추가 계약·약정이 필요한 경우, 입력된 대표자의 이메일로 전자 약정서가 발송됩니다.</p>
						<input type="text" name="fcRepEmail" class="form-control" id="tfcRepEmail" placeholder="대표자 이메일 입력" >
						<br>
					...
					<div class="calculate-wrap">
						...
						<div class="calculate-input">
							...
							<input type="text" name="fcRepBirth" class="form-control" id="tfcRepBirth" placeholder="주민등록번호 입력" >
							<br>
							 -->
							...
							<input type="text" name="fcMail" class="form-control" id="tfcMail" placeholder="이메일 입력" >
							<br>
							...
							<select class="form-select" id="tfcBankName" name="fcBankName">
							...
							<input type="number" name="fcBankNo" class="form-control" id="tfcBankNo" placeholder="계좌번호 입력" >
							<p>특수문자(-)없이 숫자만 입력해주세요.</p>
							<input type="text" name="fcBankAccount" class="form-control" id="tfcBankAccount" placeholder="예금주명 입력" >
	...
	<script>
		...
		const saveChk = [1,1,1,1,1,0,0,0,0]
		...
		$("[type=submit]").on("click",function(e){
			if(confirm("변경 내용을 저장하시겠습니까 ? ")){
				...
				*/
				...
				if(tEmail.match(mailReg) == null || tEmail == "" ){
					saveChk[5] = 0;
				}else{
					saveChk[5] = 1;
				}
				...
				//8. 통장사본 체크
				const tbFilepath = $(".upfile").val();
				const imageStatus2 = $("[name=imageStatus]").val();
		...
		$(document).ready(function(){
			...
			<c:if test="${fc ne null}">
				...
				$("#tfcRepCategory").val("${fc.fcRepCategory}");
				$("#tfcRepName").val("${fc.fcRepName}");
				$("#tfcRepEmail").val("${fc.fcRepEmail}");
				*/
				$(".form-label1>span").css("display","none");
				$(".cancel").css("display","block");
				//$("#image1").val("image 등록 완료");
				$("#image2").val("image 등록 완료");
				//$("#tfcRepBirth").val("${fc.fcRepBirth}");
				$("#tfcMail").val("${fc.fcMail}");
				$("#tfcBankName").val("${fc.fcBankName}");
				$("#tfcBankNo").val("${fc.fcBankNo}");
				$("#tfcBankAccount").val("${fc.fcBankAccount}");
			</c:if>
		});
	</script>
